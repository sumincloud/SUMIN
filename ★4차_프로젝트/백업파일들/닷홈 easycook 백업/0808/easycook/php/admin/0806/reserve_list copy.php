<!DOCTYPE html>
<html lang="ko">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>강사 | 이지쿡</title>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta name="keywords" content="요리 자격증, 요리 강의, 커피 자격증, 커피 강의, 베이커리 자격증, 베이커리 강의, 온라인 요리 수업, 요리 학원, 커피 학원, 베이커리 학원, 이지쿡, easycook, 요리 수강, 커피 수강, 베이킹 수강, 요리 온라인 강의, 커피 온라인 강의, 베이킹 온라인 강의, 요리 레시피, 베이킹 레시피">
  <meta name="description" content="이지쿡에서 요리, 커피, 베이커리 자격증과 수강, 온라인 강의를 배워보세요. 전문 강사진의 체계적인 교육으로 전문가가 되는 길을 안내합니다.">
  <meta name="Robots" content="Index, Follow">
  <meta http-equiv="Subject" content="이지하게 배우자, 이지쿡">
  <meta http-equiv="Title" content="이지쿡">
  <title>실습실 | 이지쿡</title>
  <?php include('header.php');   ?>
  <main>
    <section class="m-center m-auto mb-5 class_size">
      <!-- 부스러기 -->
      <p class="bread"><a href="index.php">홈</a> &#x003E; 학원 &#x003E; <span style="font-weight:bold">실습실</span></p>

      <!-- 제목 -->
      <h2 class="text-center mt-4 mb-4">실습실 예약 현황</h2>
      
      <!-- 데이터 가져오기 -->
      <?php
      // 101호
      $sql = "SELECT start, COUNT(*) FROM room  WHERE room_date = '2024-07-30' AND room = '101' GROUP BY start";
      $result = mysqli_query($conn, $sql);
      $db = mysqli_fetch_array($result);

      // 102호
      $sql2 = "select * from room where room_date = '2024-07-30' and room='102'";
      $result2 = mysqli_query($conn, $sql2);
      $db2 = mysqli_fetch_array($result2);

      // 8시
      // 9시
      // 10시
      // 11시
      // 12시
      // 1시
      // 2시
      // 3시
      // 4시

      echo $db[0]; //no
      echo $db[1]; //class_no
      echo $db[2]; //room
      echo $db[3]; //room_date
      echo $db[4]; //start
      echo $db[5]; //end
      echo $db[6]; //id
      echo $db[7]; //name
      echo $db[8]; //datetime
      ?>

      <!-- 실습실 예약 현황 보기 -->
      <article>
        <h3>예약현황 보기</h3>
        <!-- 날짜 선택 -->
        <div class="mb-3">
          <label for="t_name" class="col-sm-2 col-form-label">날짜</label>
          <div>
            <input type="date" class="form-control" id="time" name="time" >
          </div>
        </div>
        <div class="admin_reserve">
          <!-- 실습실1 -->
          <div class="mb-3 admin_reserve_con">
            <p>실습실 101호</p>
            <div>
              <p>사용시간</p>
              <p>예약현황</p>
            </div>
            <div class="reserve_time" >
              <input type="checkbox" id="time1">
              <label for="time1">
                <ul>
                  <li>08:00 ~ 09:00</li>
                  <li><span style="color:var(--red); font-weight:bold;"><?php echo $db[1]?></span> / 8</li>
                </ul>
              </label>
              <div class="reserve_p">
                <ul>
                  <li>1이초보</li>                
                  <li>2이초보</li>                
                  <li>3이초보</li>                
                  <li>4이초보</li>                
                  <li>5이초보</li>                
                  <li>6이초보</li>                
                  <li>7이초보</li>                
                  <li>8이초보</li>             
                </ul>
              </div>
            </div>
            <div class="reserve_time">
              <input type="checkbox" id="time2">
              <label for="time2">
                <ul>
                  <li>09:00 ~ 10:00</li>
                  <li><span style="color:var(--red); font-weight:bold;"><?php echo $db[2];?></span> / 8</li>
                </ul>
              </label>
              <div class="reserve_p">
                <ul>
                  <li>1이초보</li>                
                  <li>2이초보</li>                
                  <li>3이초보</li>                
                  <li>4이초보</li>                
                  <li>5이초보</li>                
                  <li>6이초보</li>                
                  <li>7이초보</li>                
                  <li>8이초보</li>             
                </ul>
              </div>
            </div>
            <div class="reserve_time">
              <input type="checkbox" id="time3">
              <label for="time3">
                <ul>
                  <li>09:00 ~ 10:00</li>
                  <li><span style="color:var(--red); font-weight:bold;"><?php echo $db[3];?></span> / 8</li>
                </ul>
              </label>
              <div class="reserve_p">
                <ul>
                  <li>1이초보</li>                
                  <li>2이초보</li>                
                  <li>3이초보</li>                
                  <li>4이초보</li>                
                  <li>5이초보</li>                
                  <li>6이초보</li>                
                  <li>7이초보</li>                
                  <li>8이초보</li>             
                </ul>
              </div>
            </div>
            <div class="reserve_time">
              <input type="checkbox" id="time4">
              <label for="time4">
                <ul>
                  <li>09:00 ~ 10:00</li>
                  <li>2/8</li>
                </ul>
              </label>
              <div class="reserve_p">
                <ul>
                  <li>1이초보</li>                
                  <li>2이초보</li>                
                  <li>3이초보</li>                
                  <li>4이초보</li>                
                  <li>5이초보</li>                
                  <li>6이초보</li>                
                  <li>7이초보</li>                
                  <li>8이초보</li>             
                </ul>
              </div>
            </div>
            <div class="reserve_time">
              <input type="checkbox" id="time5">
              <label for="time5">
                <ul>
                  <li>09:00 ~ 10:00</li>
                  <li>2/8</li>
                </ul>
              </label>
              <div class="reserve_p">
                <ul>
                  <li>1이초보</li>                
                  <li>2이초보</li>                
                  <li>3이초보</li>                
                  <li>4이초보</li>                
                  <li>5이초보</li>                
                  <li>6이초보</li>                
                  <li>7이초보</li>                
                  <li>8이초보</li>             
                </ul>
              </div>
            </div>
            <div class="reserve_time">
              <input type="checkbox" id="time6">
              <label for="time6">
                <ul>
                  <li>09:00 ~ 10:00</li>
                  <li>2/8</li>
                </ul>
              </label>
              <div class="reserve_p">
                <ul>
                  <li>1이초보</li>                
                  <li>2이초보</li>                
                  <li>3이초보</li>                
                  <li>4이초보</li>                
                  <li>5이초보</li>                
                  <li>6이초보</li>                
                  <li>7이초보</li>                
                  <li>8이초보</li>             
                </ul>
              </div>
            </div>
            <div class="reserve_time">
              <input type="checkbox" id="time7">
              <label for="time7">
                <ul>
                  <li>09:00 ~ 10:00</li>
                  <li>2/8</li>
                </ul>
              </label>
              <div class="reserve_p">
                <ul>
                  <li>1이초보</li>                
                  <li>2이초보</li>                
                  <li>3이초보</li>                
                  <li>4이초보</li>                
                  <li>5이초보</li>                
                  <li>6이초보</li>                
                  <li>7이초보</li>                
                  <li>8이초보</li>             
                </ul>
              </div>
            </div>
            <div class="reserve_time">
              <input type="checkbox" id="time8">
              <label for="time8">
                <ul>
                  <li>09:00 ~ 10:00</li>
                  <li>2/8</li>
                </ul>
              </label>
              <div class="reserve_p">
                <ul>
                  <li>1이초보</li>                
                  <li>2이초보</li>                
                  <li>3이초보</li>                
                  <li>4이초보</li>                
                  <li>5이초보</li>                
                  <li>6이초보</li>                
                  <li>7이초보</li>                
                  <li>8이초보</li>             
                </ul>
              </div>
            </div>
            <div class="reserve_time">
              <input type="checkbox" id="time9">
              <label for="time9">
                <ul>
                  <li>09:00 ~ 10:00</li>
                  <li>2/8</li>
                </ul>
              </label>
              <div class="reserve_p">
                <ul>
                  <li>1이초보</li>                
                  <li>2이초보</li>                
                  <li>3이초보</li>                
                  <li>4이초보</li>                
                  <li>5이초보</li>                
                  <li>6이초보</li>                
                  <li>7이초보</li>                
                  <li>8이초보</li>             
                </ul>
              </div>
            </div>
          </div>

          <!-- 실습실2 -->
          <div class="mb-3 admin_reserve_con">
            <p>실습실 102호</p>
            <div>
              <p>사용시간</p>
              <p>예약현황</p>
            </div>
            <div class="reserve_time" >
              <input type="checkbox" id="time1">
              <label for="time1">
                <ul>
                  <li>08:00 ~ 09:00</li>
                  <li>2/8</li>
                </ul>
              </label>
              <div class="reserve_p">
                <ul>
                  <li>1이초보</li>                
                  <li>2이초보</li>                
                  <li>3이초보</li>                
                  <li>4이초보</li>                
                  <li>5이초보</li>                
                  <li>6이초보</li>                
                  <li>7이초보</li>                
                  <li>8이초보</li>             
                </ul>
              </div>
            </div>
            <div class="reserve_time">
              <input type="checkbox" id="time2">
              <label for="time2">
                <ul>
                  <li>09:00 ~ 10:00</li>
                  <li>2/8</li>
                </ul>
              </label>
              <div class="reserve_p">
                <ul>
                  <li>1이초보</li>                
                  <li>2이초보</li>                
                  <li>3이초보</li>                
                  <li>4이초보</li>                
                  <li>5이초보</li>                
                  <li>6이초보</li>                
                  <li>7이초보</li>                
                  <li>8이초보</li>             
                </ul>
              </div>
            </div>
            <div class="reserve_time">
              <input type="checkbox" id="time3">
              <label for="time3">
                <ul>
                  <li>09:00 ~ 10:00</li>
                  <li>2/8</li>
                </ul>
              </label>
              <div class="reserve_p">
                <ul>
                  <li>1이초보</li>                
                  <li>2이초보</li>                
                  <li>3이초보</li>                
                  <li>4이초보</li>                
                  <li>5이초보</li>                
                  <li>6이초보</li>                
                  <li>7이초보</li>                
                  <li>8이초보</li>             
                </ul>
              </div>
            </div>
            <div class="reserve_time">
              <input type="checkbox" id="time4">
              <label for="time4">
                <ul>
                  <li>09:00 ~ 10:00</li>
                  <li>2/8</li>
                </ul>
              </label>
              <div class="reserve_p">
                <ul>
                  <li>1이초보</li>                
                  <li>2이초보</li>                
                  <li>3이초보</li>                
                  <li>4이초보</li>                
                  <li>5이초보</li>                
                  <li>6이초보</li>                
                  <li>7이초보</li>                
                  <li>8이초보</li>             
                </ul>
              </div>
            </div>
            <div class="reserve_time">
              <input type="checkbox" id="time5">
              <label for="time5">
                <ul>
                  <li>09:00 ~ 10:00</li>
                  <li>2/8</li>
                </ul>
              </label>
              <div class="reserve_p">
                <ul>
                  <li>1이초보</li>                
                  <li>2이초보</li>                
                  <li>3이초보</li>                
                  <li>4이초보</li>                
                  <li>5이초보</li>                
                  <li>6이초보</li>                
                  <li>7이초보</li>                
                  <li>8이초보</li>             
                </ul>
              </div>
            </div>
            <div class="reserve_time">
              <input type="checkbox" id="time6">
              <label for="time6">
                <ul>
                  <li>09:00 ~ 10:00</li>
                  <li>2/8</li>
                </ul>
              </label>
              <div class="reserve_p">
                <ul>
                  <li>1이초보</li>                
                  <li>2이초보</li>                
                  <li>3이초보</li>                
                  <li>4이초보</li>                
                  <li>5이초보</li>                
                  <li>6이초보</li>                
                  <li>7이초보</li>                
                  <li>8이초보</li>             
                </ul>
              </div>
            </div>
            <div class="reserve_time">
              <input type="checkbox" id="time7">
              <label for="time7">
                <ul>
                  <li>09:00 ~ 10:00</li>
                  <li>2/8</li>
                </ul>
              </label>
              <div class="reserve_p">
                <ul>
                  <li>1이초보</li>                
                  <li>2이초보</li>                
                  <li>3이초보</li>                
                  <li>4이초보</li>                
                  <li>5이초보</li>                
                  <li>6이초보</li>                
                  <li>7이초보</li>                
                  <li>8이초보</li>             
                </ul>
              </div>
            </div>
            <div class="reserve_time">
              <input type="checkbox" id="time8">
              <label for="time8">
                <ul>
                  <li>09:00 ~ 10:00</li>
                  <li>2/8</li>
                </ul>
              </label>
              <div class="reserve_p">
                <ul>
                  <li>1이초보</li>                
                  <li>2이초보</li>                
                  <li>3이초보</li>                
                  <li>4이초보</li>                
                  <li>5이초보</li>                
                  <li>6이초보</li>                
                  <li>7이초보</li>                
                  <li>8이초보</li>             
                </ul>
              </div>
            </div>
            <div class="reserve_time">
              <input type="checkbox" id="time9">
              <label for="time9">
                <ul>
                  <li>09:00 ~ 10:00</li>
                  <li>2/8</li>
                </ul>
              </label>
              <div class="reserve_p">
                <ul>
                  <li>1이초보</li>                
                  <li>2이초보</li>                
                  <li>3이초보</li>                
                  <li>4이초보</li>                
                  <li>5이초보</li>                
                  <li>6이초보</li>                
                  <li>7이초보</li>                
                  <li>8이초보</li>             
                </ul>
              </div>
            </div>
          </div>
        </div>
      </article>
  <?php include('footer.php');  ?>
