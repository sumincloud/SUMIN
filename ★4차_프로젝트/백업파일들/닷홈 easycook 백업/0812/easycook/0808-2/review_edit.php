...
<body>
  ...
  <section class="review_write">
      <p class="bread_c">홈&#62; 마이페이지&#62; <b>후기수정</b></p>
      <h2>후기수정</h2>
      <article class="academy_view">
        <ul class="card-list">
          ...
          <li>
            <div>
              <!-- 강의 썸네일 이미지 -->
              <a href="#" title="상세페이지로 이동">
                <img src="./uploads/class_main/<?php echo $row2['thumnail_img']; ?>" alt="강의 썸네일 사진">
              </a>
              <!-- 강의 이름 -->
              <div>
                <h2>
                  <a href="./cook_academy_detail.php?class_no=<?= $row2['class_no']; ?>" title="상세페이지로 이동">
                    <?php echo $row2['name']; ?>
                  </a>
                </h2>
      ...
      <input type='hidden' name='class_no' value='<?php echo $class_no?>'>
        ...
        <article class="re_view">
          ...
          <h2>후기 내용</h2>
          <textarea class="" name="re_view" value="re_view" maxlength="255" required><?php echo htmlspecialchars($row['review']) ?></textarea>
        </article>
