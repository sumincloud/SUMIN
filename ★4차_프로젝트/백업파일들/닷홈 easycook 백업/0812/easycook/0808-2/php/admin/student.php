    ...
    <article class="mt-2 mb-2 scrollable" >
      ...
      <table class="table table-striped text-center student table-hover" style="font-size:12px;">
        ...
        echo "<thead>"; // 테이블 헤더 출력
          echo '<tr style="border-top: 3px solid black; line-height:13px;">';
            echo '<th>No.</th>';
            echo '<th>상태</th>';
            echo '<th>학생</th>';
            echo '<th>출석</th>';
          ...
          while ($student = $result2->fetch_assoc()) {
            ...
            $count_a = $interval2->days - $row_att2[0] + 1;
                ...
                echo "<tr style='line-height:13px;'>";
                  ...
                  echo "</td>";
                  echo "<td>". $student_id . $student['name']. "</td>"; //아이디
                  echo "<td>" . round($attend_percent, 0). "%</td>"; //출석률
                  if($count_a>0){
                    echo "<td><span style='color:var(--red);'>" . $count_a. "</span></td>"; //결석일 
                    }else{
                    echo "<td></td>"; //결석일 
                    };
                  if($attendance_t > $attendance_time){
                    echo "<td><span style='color:var(--red);'>지각</span></td>"; //지각일 
                    }else{
                    echo "<td></td>"; //지각일
                    }
                  ...
                  while ($currentDate <= $endDate) {
                      ...
                      // echo $date_str."<br>" ;   //2024-08-06
                      // echo $student_id;    //student1
                      // echo $class_no; //2
                      $sql_attendance = "
                      ...
                      $row_attendance = mysqli_fetch_array($result_attendance);      // echo $row_attendance['0'];
                      
                      // 출석 상태를 확인
                      $status = $row_attendance[0] > 0 ? "&#x3000;" : "" ;                      
                      $bg_color = $status == "&#x3000;" ? "var(--green)" : "var(--red)"; // 출석이면 초록색, 결석이면 빨간색

                      echo "<td style='background-color: $bg_color;'>$status</td>";
                      $currentDate->modify('+1 day'); // 날짜 증가
                  }
                echo "</tr>";
                $count++;
