  ...
  <main>
    ...
    <form name="검색하기" method="post" action="./notice_search.php" onsubmit="return form_check()">
      ...
      <div class="con mt-2 mb-2 article" style="position:relative;">
        <!-- 검색버튼 -->
        <div class="mb-2" style="width: 230px; height: 40px;  border-radius: 30px; border: 1px solid var(--gray); position:absolute; right: 0px; top: -48px;">
          <input type="search" placeholder="제목+내용으로 찾기" aria-label="제목+내용으로 찾기"
          class="admin_search" id="search" name="search">
          ...
          <tbody>
            ...
            <?php
              ...
              while($db_b = mysqli_fetch_array($result_b)){
                ...
                echo "<tr>";
                  echo "<td style='width: 10%;' class='text-center'>".$db_b[0]."</td>";
                  echo "<td style='width: 60%; position:relative;'>
                          <span>".$db_b[2]."</span>
                          <span class='span_subtitle'>".$db_n[1]."</span></td>";
                  echo "<td style='width: 20%;' class='text-center'>".date("y-m-d",strtotime($db_b[5]))."</td>";
                  echo "<td style='width: 10%;' class='text-center'><a href='notice_list.php?class_no=".$db_b[1]."' title='강의실 바로가기'><span class='span_title' style='margin: 0 auto;'>강의실</span></a></td>";
                echo "</tr>";
        ...
        <nav aria-label="페이지네이션" class="padding50 mt-3 mb-3" style="position: relative;">
          ...
          <?php //페이지네이션이 들어가는 곳
            //이전페이지
            if($page <= 1){ ?> 
              <li class="page-item"><a href="notice_total.php?page=<?php echo ($page-1); ?>" class="page-link"><i class="bi bi-chevron-left"></i></a></li>
              <?php } 
              else{ ?> 
              <li class="page-item"><a href="notice_total.php?page=<?php echo ($page-1); ?>" class="page-link "><i class="bi bi-chevron-left"></i></a></li>
              <?php };
          ...
          <?php //여기서부터 페이지 번호출력하기
            for($print_page=$s_pageNum;$print_page<=$e_pageNum;$print_page++){?>
              <li class="page-item"><a href="notice_total.php?page=<?php echo $print_page; ?>" class="page-link">
                <?php echo $print_page ?>
              </a></li>
            <?php }; ?>  
            ...
            <?php if($page>=$total_page){ ?>
              <li class="page-item"><a href="notice_total.php?page=<?php echo $total_page; ?>" title="다음페이지로" class="page-link"><i class="bi bi-chevron-right"></i></i></a></li>
            <?php }else{ ?>
              <li class="page-item"><a href="notice_total.php?page=<?php echo ($page+1); ?>" class="page-link ">
