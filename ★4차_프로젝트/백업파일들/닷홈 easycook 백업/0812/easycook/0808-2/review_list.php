<?php
  include('./php/include/dbconn.php');
    ...
    //여기는 리뷰 불러오는곳
    $sql = "SELECT * FROM review WHERE id='$id' order by datetime desc limit $start, $list_num; ";
  ...
  <main>
    <section class="review_list">
      ...
      <nav aria-label="페이지네이션">
        ...
        <?php //이전페이지
          if($page <= 1){ ?> 
            <li class="page-item"><a href="review_list.php?no=<?php echo $class_no;?>&page=<?php echo ($page-1); ?>" class="page-link"><i class="bi bi-chevron-left"></i></a></li>
            <?php } 
            else{ ?> 
            <li class="page-item"><a href="review_list.php?no=<?php echo $class_no;?>&page=<?php echo ($page-1); ?>" class="page-link "><i class="bi bi-chevron-left"></i></a></li>
            <?php }; ?> 
        <?php //여기서부터 페이지 번호출력하기
          for($print_page=$s_pageNum;$print_page<=$e_pageNum;$print_page++){?>
            <li class="page-item"><a href="review_list.php?no=<?php echo $class_no;?>&page=<?php echo $print_page; ?>" class="page-link">
              <?php echo $print_page ?>
            </a></li>
          <?php }; ?>  
          <!-- 다음 버튼 나오는 곳 -->
          <?php if($page>=$total_page){ ?>
            <li class="page-item"><a href="review_list.php?no=<?php echo $class_no;?>&page=<?php echo $total_page; ?>" title="다음페이지로" class="page-link"><i class="bi bi-chevron-right"></i></a></li>
          <?php }else{ ?>
            <li class="page-item"><a href="review_list.php?no=<?php echo $class_no;?>&page=<?php echo ($page+1); ?>" class="page-link" ><i class="bi bi-chevron-right"></i></a></li>
        <?php };   ?>    
        </ul>
      </nav>  
    </section>
