...
<main>
  <section class="m-center m-auto mb-5 class_size">
    ...
    <table class="table table-hover text-center" style="font-size:var(--fs-small)">
  ...
  <thead>
    ...
    while ($class_c = mysqli_fetch_array($result_c)) {
      ...
      echo '<tr style="border-top: 3px solid black; line-height:50px;">';
        echo '<th style="width:50px;">번호</th>';
        echo '<th style="width:50px;">학생명</th>';
        echo '<th style="width:50px;">결석</th>';
        echo '<th style="width:80px;">상태</th>';

      // 시작일부터 종료일까지의 날짜 출력
      while ($startDate <= $endDate) {
        // 날짜가 오늘 날짜보다 큰 경우에만 출력
        if ($startDate <= new DateTime()) {
          echo '<th style="width:50px;">' . $startDate->format('m/d') . '</th>';
        }
